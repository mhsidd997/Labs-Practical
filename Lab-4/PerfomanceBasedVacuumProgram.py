...
class RationalPerformanceVaccumCleanerEnvironment(Environment.Environment):
    ''' classdocs ''' 
        ...
        for _ in range(0,n):
            ...
            if res == "clean":
                ...
                if self.currentRoom == self.rooms[0]:
                    ...
                    self.currentRoom = self.rooms[self.rooms.index(self.currentRoom)-1]
                self.score -= 1
            elif self.currentRoom.status == "dirty":
                self.score -= 10
            elif res == "stop":
                self.displayAction()
                break
...
if __name__ == '__main__':
    ...
    s = checkInput(x,'s')  
    vcagent = VaccumAgent() 
    env = RationalPerformanceVaccumCleanerEnvironment(vcagent,r) 
    env.executeStep(s)
